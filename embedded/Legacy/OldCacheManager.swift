...
class MapCacheManager {
  ...
  func getTileFromDatabase(url: URL, forceDownscale: Bool = false) throws ->TileResponse? {
    guard let q = self.database else {
      throw RuntimeError.invalidValue("Database not found")
    }
    ...
    //    }
    return try q.read { db throws -> TileResponse? in
      
      let sql: Statement?
      var args: StatementArguments = ["path": matchParams.templateURL]
      switch matchParams.cacheType {
      case .tile(let x, let y, let z):
        sql = try db.makeStatement(sql: getTile)
        args = [
          "x": x,
          "y": y,
          "z": z,
          "path": matchParams.templateURL
        ]
      case .resource:
        sql = try db.makeStatement(sql: getResource)
      }
      guard let stmt = sql else {
        return nil
      }
      
      let tileData = try Row.fetchOne(stmt, arguments: args)
      guard let row = tileData,
            let data: Data = row["data"],
            let urlTemplate: String = row["url"],
            let compressed: Bool = row["compressed"]
      else {
        return nil
      }
...
SELECT
 ...
 compressed
FROM tiles
WHERE (url_template = :path
   OR url_template = replace(:path, '{ratio}', '@2x')
   OR url_template = replace(:path, '{ratio}', ''))
  AND x = :x
  AND y = :y
  AND z = :z
LIMIT 1
